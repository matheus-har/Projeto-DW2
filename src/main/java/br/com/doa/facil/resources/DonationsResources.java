...
public class DonationsResources {
    ...
    @GetMapping
  @PreAuthorize("hasAuthority('ROLE_SEARCH_DONATION')")
  ...
  @PreAuthorize("hasAuthority('ROLE_SEARCH_DONATION')")
    public ResponseEntity<Donations> findById(@PathVariable Long id){
        ...
        return donations.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
   @PreAuthorize("hasAuthority('ROLE_REGISTER_DONATION')")
    public Donations create(@Valid @RequestBody Donations donations) {
        return donationsRepository.save(donations);
    }

    @DeleteMapping("/{id}")
   @PreAuthorize("hasAuthority('ROLE_REMOVE_DONATION')")
